...
void livox_pcl_cbk(const livox_ros_driver2::CustomMsg::ConstPtr &msg)
{
    ...
    p_pre->process(msg, ptr);
        ...
        PointCloudXYZI::Ptr laserCloud_tran_grav1(new PointCloudXYZI());
        int icout = 0;
        ...
        for (size_t i = 0; i < size_init_cloud; i++)
        {
            ...
            laserCloud_tran_grav1->points.push_back(ptr->points[i]);
            

        }
        laserCloud_tran_grav1->width = laserCloud_tran_grav1->points.size();  
        laserCloud_tran_grav1->height = 1;  
        laserCloud_tran_grav1->is_dense = true; 
...
void set_oula_posestamp(T & out)
{
    ...
    Eigen::Vector3f p1grav(rotation_quaternion_grav*p1 );

    out.pose.position.x = p1grav(0);
    out.pose.position.y = p1grav(1);
    out.pose.position.z = p1grav(2);
    Eigen::Quaternionf q1(geoQuat.w, geoQuat.x, geoQuat.y, geoQuat.z);
    Eigen::Quaternionf q2 = rotation_quaternion_grav*q1;

    double r,p,y;
    toEulerAngle(q2,r,p,y);
    out.pose.orientation.x = r-(rotation_angle_x/3.14*180);
    out.pose.orientation.y = p-(rotation_angle_y/3.14*180);
...
//------------------------------------------------------------------------------------------------------
    ...
    std::cout << "Rotation x angle:\n"
        << rotation_angle_x << std::endl;

    std::cout << "Rotation y angle:\n"
        << rotation_angle_y << std::endl;
