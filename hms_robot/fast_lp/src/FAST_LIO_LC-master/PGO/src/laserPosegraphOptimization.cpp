...
void pubPath( void )
{
    ...
    for (int node_idx=0; node_idx < recentIdxUpdated; node_idx++) // -1 is just delayed visualization (because sometimes mutexed while adding(push_back) a new one)
    {
        ...
        odomAftPGOthis.child_frame_id = "body";
    ...
    br.sendTransform(tf::StampedTransform(transform, odomAftPGO.header.stamp, "camera_init", "body"));
} // pubPath
...
gtsam::Pose3 doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    // 提取当前关键帧特征点集合，降采样
    loopFindNearKeyframes(cureKeyframeCloud, _curr_kf_idx, 3);
...
bool detectLoopClosureDistance(int *loopKeyCur, int *loopKeyPre)
{
    ...
    for(int i = 0; i < pointSearchIndLoop.size(); ++i)
    {
        ...
        if ( abs( keyframeTimes[id] - keyframeTimes[*loopKeyCur] ) > historyKeyframeSearchTimeDiff && pointSearchSqDisLoop[id]< 5 )
        {
            *loopKeyPre = id;
            break;
        }
...
void sigintHandler(int sig) {
    ROS_INFO("Saving point cloud on SIGINT...");
    if (acc_grav())
    {   
        int size = laserCloudMapPGO->points.size();
        pcl::PointCloud<PointType>::Ptr laserCloud_tran_grav(new pcl::PointCloud<PointType>(size, 1));
        
        for (int i = 0; i < size; i++)
        {
             pointstran_grav(&laserCloudMapPGO->points[i], \
                                &laserCloud_tran_grav->points[i]);
        }
        
        pcl::io::savePCDFile(std::string(ROOT_)+"pcd/update_map.pcd", *laserCloud_tran_grav);
    }
    ros::shutdown();
}
